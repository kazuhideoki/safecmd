...
- **強制上書きモード**: `-f` で上書き時エラーを抑制して処理継続（エージェント互換）
...
本フェーズの最低限インターフェースは **`-f`, `-n`, `-r/-R`** とする。
...
2. **上書き時の動作**
   ...
   - `-f` はエージェント互換のため受理し、上書き時の挙動を明示できるようにする
   - `-n` 指定時は既存ファイルを上書きしない（スキップ）
   - `-f` と `-n` の同時指定はエラー（終了コード `1`）
...
# 既存ファイルを上書きしない
safecmd cp -n config.toml backup/config.toml
